...
class DirectMessageService: ObservableObject {
    ...
    func fetchUser(by userId: String) async throws -> User {
        print("[DirectMessageService] fetchUser çağrıldı - userId: \(userId)")
        do {
            let document = try await db.collection("users").document(userId).getDocument()
            if !document.exists {
                print("[DirectMessageService] Kullanıcı dokümanı bulunamadı - userId: \(userId)")
                throw NSError(
            ...
            guard let data = document.data() else {
                print("[DirectMessageService] Kullanıcı verisi boş - userId: \(userId)")
                throw NSError(
                    domain: "DirectMessageService", 
                    code: 3, 
            ...
            if !user.isValid {
                print("[DirectMessageService] Geçersiz kullanıcı verisi - userId: \(userId)")
                throw NSError(
                    domain: "DirectMessageService", 
                    code: 4, 
            ...
            print("[DirectMessageService] Kullanıcı başarıyla getirildi - userId: \(userId), username: \(user.username)")
            return user
        } catch let error as NSError {
            if error.domain == "DirectMessageService" {
                // Bizim tanımladığımız hataları tekrar fırlat
                throw error
            } else {
                // Firestore hatalarını sararak daha anlaşılır hale getir
                print("[DirectMessageService] Firestore hatası - userId: \(userId), error: \(error.localizedDescription)")
                throw NSError(
                    domain: "DirectMessageService", 
                    code: 1, 
    ...
    func searchUsers(with searchText: String, limit: Int = 20) async throws -> [User] {
        print("🔍 DirectMessageService.searchUsers çağrıldı - Arama metni: '\(searchText)', Limit: \(limit)")
        ...
        print("[DirectMessageService] Firestore'da userId ile arama başlatılıyor: \(searchText)")
        if let userByIdSnapshot = try? await db.collection("users").document(searchText).getDocument(),
           userByIdSnapshot.exists {
            ...
            if userById.isValid {
                ...
                print("[DirectMessageService] Firestore userId ile eşleşen kullanıcı bulundu: \(userById.username)")
            }
        }
        
        // 2. usernameLower ile arama
        print("[DirectMessageService] Firestore'da usernameLower ile arama başlatılıyor: \(lowercaseSearch)")
        ...
        print("[DirectMessageService] Firestore usernameLower arama sonucu: \(usernameQuery.documents.count) kullanıcı")
        ...
        // 3. Email ile arama
        print("[DirectMessageService] Firestore'da email ile arama başlatılıyor: \(lowercaseSearch)")
        ...
        print("[DirectMessageService] Firestore email arama sonucu: \(emailQuery.documents.count) kullanıcı")
        ...
        print("[DirectMessageService] Arama tamamlandı, sonuç sayısı: \(uniqueUsers.count)")
