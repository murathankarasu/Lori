...
class DirectMessageService: ObservableObject {
    ...
    func sendImageMessage(to conversationId: String, image: UIImage, senderId: String, receiverId: String) async throws {
        do {
            // Önce görseli yükle ve URL'ini al
            let imageURL = try await mediaService.uploadImage(image, conversationId: conversationId)
            ...
            // Mesajı gönder
            try await sendMessage(to: conversationId, message: message)
