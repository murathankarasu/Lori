...
public class HateSpeechService {
    ...
    private func loadBannedWords() {
        ...
        do {
            ...
            // Debug için yasaklı kelimeleri göster
            print("📖 Bazı yasaklı kelimeler: \(Array(bannedWords.keys.prefix(5)))")
            // "hate" yasaklı mı kontrol et
            if let category = bannedWords["hate"] {
                print("📖 'hate' kelimesi yasaklı: \(category)")
            } else {
                print("📖 'hate' kelimesi yasaklı değil")
            }
            // "fuck" yasaklı mı kontrol et
            if let category = bannedWords["fuck"] {
                print("📖 'fuck' kelimesi yasaklı: \(category)")
            } else {
                print("📖 'fuck' kelimesi yasaklı değil")
            }
        } catch {
    ...
    func checkLocalHateSpeech(_ text: String) -> (containsHateSpeech: Bool, category: String?, word: String?) {
        print("\n💬 YEREL NEFRET SÖYLEMİ KONTROLÜ BAŞLADI: \"\(text)\"")
        let words = text.lowercased().components(separatedBy: .whitespacesAndNewlines)
        print("💬 Kelimeler: \(words)")
        
        for word in words {
            print("💬 Kelime kontrolü: \"\(word)\"")
        ...
        print("💬 YEREL NEFRET SÖYLEMİ KONTROLÜ: Hiç yasaklı kelime bulunamadı")
        return (false, nil, nil)
    }
    
    func checkHateSpeech(text: String) async throws -> HateSpeechResponse {
        print("\n🔴 NEFRET SÖYLEMİ KONTROLÜ BAŞLADI 🔴")
        ...
        do {
            ...
            print("🔴 NEFRET SÖYLEMİ KONTROLÜ TAMAMLANDI 🔴\n")
