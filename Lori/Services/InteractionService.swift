...
class InteractionService {
        ...
        if isLiked {
            ...
            likeRef.delete { error in
                ...
                postRef.updateData(["likes": FieldValue.increment(Int64(-1))]) { error in
                    ...
                    Task {
                        do {
                            let emotionAnalysis = try await self.emotionService.analyzeEmotion(text: post.content)
        ...
        do {
            ...
            try commentRef.setData(from: comment) { error in
                ...
                postRef.updateData(["comments": FieldValue.increment(Int64(1))]) { error in
                    ...
                    Task {
                        do {
                            let emotionAnalysis = try await self.emotionService.analyzeEmotion(text: comment.content)
                            try await self.userEmotionService.saveInteraction(
                                userId: userId,
                                postId: postId,
    ...
    func recordViewInteraction(for post: Post) {
        ...
        Task {
            do {
                let emotionAnalysis = try await emotionService.analyzeEmotion(text: post.content)
