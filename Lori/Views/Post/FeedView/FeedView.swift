...
struct CustomTabBarView: UIViewControllerRepresentable {
    ...
    // Shared ViewModel Manager'ı ekliyoruz
    private let sharedViewModelManager = SharedViewModelManager.shared
    ...
    private func getTabView(for index: Int) -> some View {
        switch index {
        case 0:
            // Featured feed için shared ViewModel kullan
            return AnyView(
                FeaturedFeedView(selectedPost: $selectedPost, showPostDetail: $showPostDetail)
            )
        case 1:
            // Following feed için shared ViewModel kullan
            return AnyView(
                FollowingFeedView(selectedPost: $selectedPost, showPostDetail: $showPostDetail)
            )
        case 2:
            return AnyView(ProfileView(userId: Auth.auth().currentUser?.uid ?? "", isLoggedIn: $isLoggedIn))
        case 3:
    ...
    class Coordinator: NSObject, UITabBarControllerDelegate {
        ...
        func tabBarController(_ tabBarController: UITabBarController, didSelect viewController: UIViewController) {
            ...
            print("📱 Tab değişti: \(lastSelectedIndex) -> \(currentIndex)")
            
            // Cache durumunu debug et
            CacheManager.shared.printCacheStatus()
            
            // Aynı tab'a tekrar basılırsa scroll to top veya refresh yapabilir
            if currentIndex == lastSelectedIndex {
                switch currentIndex {
                case 0: // Featured
                    print("🔄 Featured tab'a tekrar basıldı - refresh trigger")
                    // Burada istersen refresh yapabilirsin
                    break
                case 1: // Following
                    print("🔄 Following tab'a tekrar basıldı - refresh trigger")
                    break
                default:
                    break
                }
                ...
                switch currentIndex {
                case 0: // Featured
                    print("🏠 Featured tab seçildi")
                    // ViewModel zaten cache'den yüklüyor, ekstra işlem gerekmiyor
                    break
                case 1: // Following
                    print("👥 Following tab seçildi")
                    // ViewModel zaten cache'den yüklüyor, ekstra işlem gerekmiyor
                    break
                default:
                    break
        ...
        func tabBarController(_ tabBarController: UITabBarController, shouldSelect viewController: UIViewController) -> Bool {
            // Bu fonksiyon ile seçim öncesi kontrol yapılabilir
            return true
        }
    }
}
