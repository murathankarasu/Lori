    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                ...
                Text(!response.isVerified ? "Dezenformasyon Riski" : "Güvenilir İçerik")
                    .font(.headline)
                    .foregroundColor(!response.isVerified ? .orange : .green)
...
#Preview {
    CreatePostDisinformationCheckSummaryView(
        response: DisinformationResponse(
            ...
            confidence: 0.85,
            explanation: "Bu içerik güvenilir kaynaklarda doğrulanamamış bilgiler içeriyor."
        )
    )
    .padding()
