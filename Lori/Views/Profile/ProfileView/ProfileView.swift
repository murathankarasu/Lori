    ...
    var body: some View {
            ...
            VStack(spacing: 0) {
                HStack {
                    ...
                    if viewModel.isCurrentUser {
                        ...
                        Button(action: { showDirectMessage = true }) {
        ...
        .sheet(isPresented: $showDirectMessage) {
            if let currentUserId = Auth.auth().currentUser?.uid {
                DirectMessageView(userId: currentUserId, selectedUserId: userId)
            }
        }
        .onAppear {
...
// Başlangıçta belirli bir kullanıcıyla yeni mesaj ekranını açan görünüm
struct DirectMessageView: View {
    ...
    var body: some View {
        NavigationView {
            DirectMessageListView(userId: userId)
                .onAppear {
                    // Burada bir gecikme ekleyerek önce DirectMessageListView'ın yüklenmesini sağlıyoruz
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                        showNewMessageViewWithSelectedUser()
                    }
                }
        }
    }
    
    private func showNewMessageViewWithSelectedUser() {
        // iOS 15+ için konformu olan ve NotificationCenter kullanmayan bir yöntem
