...
struct InterestsView: View {
    ...
    @State private var interests: [Interest] = [
        Interest(name: "Technology", icon: "laptopcomputer"),
        Interest(name: "Sports", icon: "sportscourt"),
        Interest(name: "Music", icon: "music.note"),
        Interest(name: "Art", icon: "paintpalette"),
        Interest(name: "Food", icon: "fork.knife"),
        Interest(name: "Travel", icon: "airplane"),
        Interest(name: "Books", icon: "book"),
        Interest(name: "Movies", icon: "film"),
        Interest(name: "Games", icon: "gamecontroller"),
    ...
    var body: some View {
        ...
        .alert("Log Out", isPresented: $showExitAlert) {
            ...
            Button("Log Out", role: .destructive) {
                do {
                    ...
                    alertMessage = "An error occurred while logging out: \(error.localizedDescription)"
                    showAlert = true
                }
            }
        } message: {
    ...
    private func saveInterests() {
        guard selectedInterests.count >= 3 else {
            alertMessage = "Please select at least 3 interests."
            showAlert = true
            return
        }
        ...
        Auth.auth().currentUser?.reload { error in
            if let error = error {
                alertMessage = "Could not verify user information: \(error.localizedDescription)"
                showAlert = true
                isCreatingUser = false
                return
            }
            ...
            db.collection("users").document(userId).updateData([
                ...
                if let error = error {
                    alertMessage = "Could not save interests: \(error.localizedDescription)"
                    showAlert = true
                    return
                }
                
                alertMessage = "Your interests have been successfully saved!"
                showAlert = true
                shouldNavigateToFeed = true
            }
        }
