...
struct Post: Identifiable, Codable, Hashable, Equatable {
    ...
    let timestamp: Date
    var likes: Int = 0  // Varsayılan değer 0 olacak
    var comments: [Comment] = []
    var tags: [String] = []  // Varsayılan boş dizi
    ...
    init?(id: String, data: [String: Any]) {
        ...
        guard let likes = data["likes"] as? Int else {
            ...
            // likes alanı sorunluysa, devam edeceğiz ama varsayılan 0 değerini kullanacağız
            let likes = 0
            
            // Burada diğer alanları kontrol edelim
    ...
    // Firestore'dan daha güvenli veri oluşturma için statik yardımcı
    static func create(from document: DocumentSnapshot) -> Post? {
        guard let data = document.data() else {
            print("No data found in document \(document.documentID)")
            return nil
        }
        
        // Güvenli şekilde Post oluştur
        let post = Post(id: document.documentID, data: data)
        return post
    }
} 
